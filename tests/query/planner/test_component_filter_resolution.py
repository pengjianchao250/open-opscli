"""组件筛选值解析的回归测试（渠道走授权枚举，ASIN 走字面格式）。

守住的核心行为：规划器绝不下发「用户要了筛选、模板里却没有该筛选」的可执行合同。
线上事故形态：查「渠道是傲彼瑞的所有ASIN」返回了傲创-美国、莱福特-美国的数据——
筛选值从未写入 query_template，而执行入口在 status=planned 时原样执行模板。

历史说明：本文件原先同时对拍 Skill 版与内核版规划器；Skill 本地规划器移除后，
只保留内核半边，部门筛选一节因与 test_query_plan.py 完全重复而不再保留。
"""

from __future__ import annotations

import pytest

from opscli.query.services.planner import query_plan

# 当前账号可见的渠道授权原值：「傲彼瑞」是两个地区渠道的共同前缀，
# 用户口语里的「傲彼瑞」因此不该被任何一个单独承接
CHANNEL_VALUES = ["傲彼瑞-美国", "傲彼瑞-加拿大", "傲创-美国", "莱福特-美国"]


def _channel_contract() -> dict:
    """构造一个待解析渠道筛选的 planned 合同骨架。"""
    return {
        "status": "planned",
        "query_mode": "dataset_query",
        "model_view": {"clarification_messages_zh": [], "next_action": "construct_query"},
        "execution_ref": {
            "dataset_alias": "ds_instant",
            "filter_components": [
                {
                    "field_name": "channel_name",
                    "label_zh": "渠道",
                    "component_dataset_alias": "ds_channel",
                    "component_table_id": 7,
                }
            ],
            "query_template": {
                "tableId": 1,
                "dimensions": [],
                "metrics": [],
                "filters": [],
            },
        },
    }


def _resolve(query: str, enum_values: list) -> dict:
    """调用内核组件筛选解析（枚举经 enum_fn 注入），返回处理后的合同。"""

    def enum_fn(_table_id, _field_name, *, limit):
        return enum_values

    return query_plan._resolve_component_filters(
        _channel_contract(), query, enum_fn, auto_enum=True
    )


def _channel_filters(contract: dict) -> list:
    template = (contract.get("execution_ref") or {}).get("query_template") or {}
    return [
        item for item in template.get("filters") or [] if item.get("field") == "channel_name"
    ]


def test_exact_channel_value_is_written_into_template():
    """完整等值唯一命中时把授权原值写入模板，正常放行。"""
    contract = _resolve("查渠道是傲彼瑞-美国的所有ASIN", CHANNEL_VALUES)
    assert contract["status"] == "planned"
    assert _channel_filters(contract) == [
        {"field": "channel_name", "operator": "=", "value": "傲彼瑞-美国"}
    ]


def test_ambiguous_channel_value_blocks_execution():
    """「傲彼瑞」对应两个地区渠道：必须澄清，且不得下发可执行模板。"""
    contract = _resolve("查渠道是傲彼瑞的所有ASIN", CHANNEL_VALUES)
    assert contract["status"] == "clarify_required"
    assert "query_template" not in contract["execution_ref"]
    message = " ".join(contract["model_view"]["clarification_messages_zh"])
    assert "傲彼瑞-美国" in message and "傲彼瑞-加拿大" in message


def test_bare_channel_value_is_caught_by_reverse_lookup():
    """原文不含「渠道」二字的裸值同样要被拦住。

    这是线上事故的真实形态：标签正则抽不到值，若不做枚举反查就会被当成
    「没有筛选意图」直接放行，静默返回全部渠道的数据。
    """
    contract = _resolve("查傲彼瑞的所有ASIN", CHANNEL_VALUES)
    assert contract["status"] == "clarify_required"
    assert "query_template" not in contract["execution_ref"]


def test_unique_bare_channel_value_resolves():
    """裸值唯一命中授权原值时直接锁定，不必多问一轮。"""
    contract = _resolve("查莱福特-美国的所有ASIN", CHANNEL_VALUES)
    assert contract["status"] == "planned"
    assert _channel_filters(contract) == [
        {"field": "channel_name", "operator": "=", "value": "莱福特-美国"}
    ]


def test_query_without_channel_value_passes_through():
    """原文没有任何渠道值时正常放行，不制造多余澄清。"""
    contract = _resolve("查渠道和ASIN的明细", CHANNEL_VALUES)
    assert contract["status"] == "planned"
    assert _channel_filters(contract) == []


def test_enum_failure_is_fail_closed():
    """枚举拿不到授权原值时一律阻断，绝不放行成全范围查询。"""
    contract = _resolve("查渠道是傲彼瑞的所有ASIN", [])
    assert contract["status"] == "blocked"
    assert "query_template" not in contract["execution_ref"]
    assert contract["model_view"]["component_filter_state"] in {
        "enum_unavailable",
        "enum_failed",
    }


@pytest.mark.parametrize(
    "query, expected",
    [
        # 标准 Amazon ASIN：B + 9 位
        ("查 B0FWR9Y2NV 的销量", ["B0FWR9Y2NV"]),
        # TEMU 等平台的纯数字商品 ID（实测 10/11/14 位都存在于 asin 列）
        ("查 61594716002 的销量", ["61594716002"]),
        ("查 48657686364417 的销量", ["48657686364417"]),
        # 多个值一并锁定，并统一大写
        ("查 b0fwr9y2nv 和 61594716002", ["B0FWR9Y2NV", "61594716002"]),
        # 年份、数量、limit 这类普通数字不得被误吃
        ("近30天销量，2026 年，limit 1000", []),
    ],
)
def test_asin_shapes_are_recognized(query: str, expected: list):
    """商品 ID 形态不是固定的 B0+8 位，写死会漏值并退化成静默全量。"""
    assert query_plan._extract_asin_values(query) == expected


def test_channel_label_does_not_swallow_trailing_words():
    """标签抽取必须在助词处收住，不能把「傲彼瑞的所有ASIN」整段当成筛选值。"""
    terms = ("渠道", "channel")
    assert query_plan._extract_labeled_value("查渠道是傲彼瑞的所有ASIN", terms) == "傲彼瑞"
    assert query_plan._extract_labeled_value("渠道为傲彼瑞-美国，近7天", terms) == "傲彼瑞-美国"
    # 维度点名不是筛选值
    assert query_plan._extract_labeled_value("查渠道和ASIN", terms) == ""


def test_longer_label_wins_over_prefix():
    """「渠道SKU」不能被「渠道」抢先匹配掉——标签必须按长度降序尝试。"""
    value = query_plan._extract_labeled_value(
        "查渠道SKU是ON-OB-JL-007-68157的销量", ("渠道", "渠道sku")
    )
    assert value == "ON-OB-JL-007-68157"


@pytest.mark.parametrize(
    "field_name, query, expected",
    [
        ("sell_sku", "查 ON-OB-JL-007-68157 的销量", "ON-OB-JL-007-68157"),
        ("pmc_code", "查 32.002946 的库存", "32.002946"),
        ("spu", "查 BKC-107 的销量", "BKC-107"),
        ("model", "查 COT-135-WA 的销量", "COT-135-WA"),
    ],
)
def test_code_shaped_bare_values_are_extracted(field_name, query, expected):
    """编码型字段的裸值靠形态抽取兜底（这些字段基数太大，不适合枚举反查）。"""
    spec = next(
        item for item in query_plan._ENUM_COMPONENT_SPECS if item["field_name"] == field_name
    )
    assert query_plan._extract_patterned_value(query, spec["value_pattern"]) == expected


# ---------------------------------------------------------------------------
# 二期：其余组件字段（sell_sku / 国家 / 品牌 / 销售 等）
# ---------------------------------------------------------------------------


def test_all_value_component_fields_are_covered():
    """17 个值类组件字段必须全部纳入 specs——漏一个就是一条静默漏筛的路径。

    date_id 归时间口径、platform_name 归平台范围逻辑，两者不在此表内。
    """
    covered = {item["field_name"] for item in query_plan._ENUM_COMPONENT_SPECS}
    expected = {
        "dept_name", "channel_name", "country_name", "brand_name",
        "team_username", "develop_username", "team_name", "large_team_name",
        "category", "amazon_cat", "sell_sku", "ed_sku", "model",
        "pmc_code", "spu", "product_name",
    }
    assert expected - covered == set(), f"未覆盖的组件字段：{expected - covered}"


def test_only_low_cardinality_fields_use_reverse_lookup():
    """反查只给低基数字段开：高基数字段枚举不完整，反查会给出错误的「无命中」。

    实测基数：渠道 9 / 国家 2 / 品牌 3 / 销售 12 属低基数；
    公司SKU 466、产品名称 376、渠道SKU 152 属高基数，改用形态抽取。
    """
    reverse = {
        item["field_name"]
        for item in query_plan._ENUM_COMPONENT_SPECS
        if item.get("reverse_lookup")
    }
    high_cardinality = {"sell_sku", "ed_sku", "product_name", "model", "pmc_code", "spu", "asin"}
    assert reverse & high_cardinality == set(), f"高基数字段不应开反查：{reverse & high_cardinality}"
    assert "channel_name" in reverse


@pytest.mark.parametrize(
    "query, field_name, expected",
    [
        ("查渠道SKU是ON-OB-JL-007-68157的销量", "sell_sku", "ON-OB-JL-007-68157"),
        ("查国家为美国的销量", "country_name", "美国"),
        ("查品牌是OHWILL的销量", "brand_name", "OHWILL"),
        ("查销售是史子涵的业绩", "team_username", "史子涵"),
        ("查大组为OR-C的销量", "large_team_name", "OR-C"),
        ("查物控编码是32.002946的库存", "pmc_code", "32.002946"),
    ],
)
def test_labeled_values_across_component_fields(query, field_name, expected):
    """各组件字段的标签形态都要能抽出值，抽不到就等于静默漏筛。"""
    spec = next(
        item for item in query_plan._ENUM_COMPONENT_SPECS if item["field_name"] == field_name
    )
    assert query_plan._spec_extract(spec, query) == expected


def test_spec_extract_prefers_label_over_pattern():
    """同时命中标签与形态时以标签为准，避免形态误抽走用户明确指定的值。"""
    spec = next(
        item for item in query_plan._ENUM_COMPONENT_SPECS if item["field_name"] == "sell_sku"
    )
    assert query_plan._spec_extract(spec, "查渠道SKU是ON-OB-JL-007-68157的销量") == "ON-OB-JL-007-68157"


def test_consumed_value_is_not_reclaimed_by_pattern_of_another_field():
    """一个值只能被一个字段消费：SPU 的形态不得从渠道SKU 里抠子串再澄清一次。

    实测形态：「渠道SKU是ON-OB-JL-007-68157」被 SPU 的 `[A-Z]{2,}-[0-9]{2,}`
    抠出「JL-007」，二次澄清还会撤掉已经生成的模板。
    """
    spu = next(
        item for item in query_plan._ENUM_COMPONENT_SPECS if item["field_name"] == "spu"
    )
    consumed = {query_plan._normalize_component_value("ON-OB-JL-007-68157")}
    assert query_plan._spec_extract(
        spu, "查渠道SKU是ON-OB-JL-007-68157的销量", consumed=consumed
    ) == ""
    # 未被消费时仍要能抽出
    assert query_plan._spec_extract(spu, "查 BKC-107 的销量", consumed=set()) == "BKC-107"


def test_reverse_lookup_prefers_exact_value_over_base_segment():
    """原文写了完整枚举值就锁定它，不因主段同时命中多个成员而退化成澄清。"""
    values = ["傲彼瑞-美国", "傲彼瑞-加拿大", "傲彼瑞-tiktok"]
    hits = query_plan._reverse_lookup_component_values(
        "查傲彼瑞-加拿大的所有ASIN", values, query_plan._normalize_component_value
    )
    assert hits == ["傲彼瑞-加拿大"]


def test_reverse_lookup_skips_values_contained_in_consumed_ones():
    """渠道锁定「傲彼瑞-加拿大」后，其中的「加拿大」不该再被国家反查抓走。"""
    consumed = {query_plan._normalize_component_value("傲彼瑞-加拿大")}
    assert query_plan._value_already_consumed(
        query_plan._normalize_component_value("加拿大"), consumed
    )
    assert not query_plan._value_already_consumed(
        query_plan._normalize_component_value("美国"), consumed
    )


def test_empty_enum_without_requested_value_does_not_block():
    """字段在当前账号下无授权值时跳过即可，不能把所有查询都阻断。

    「开发」在部分账号下枚举就是 0 条；早期实现把「枚举成功但为空」当成
    「枚举失败」，导致每一条查询都返回 blocked。
    """
    contract = _resolve("查渠道和ASIN的明细", [])
    # 渠道字段有显式值时才阻断；此处原文无任何筛选值，应正常放行
    assert contract["status"] == "planned"


# ---------------------------------------------------------------------------
# Task 10：枚举反查排除通用业务词主段（修复「亚马逊」被当成销售小组）
# ---------------------------------------------------------------------------


def test_reverse_lookup_skips_generic_platform_base():
    """枚举值主段是平台名时不得靠主段命中。

    实测缺陷：销售小组授权值形如「亚马逊-运营C组」，主段是「亚马逊」，
    于是任何提到亚马逊的查询都会被当成指定了销售小组，
    真实元数据下已观察到 team_name 被注入 filters。
    """
    teams = ["亚马逊-运营C组", "亚马逊-运营A组", "沃尔玛-运营B组"]
    hits = query_plan._reverse_lookup_component_values(
        "查亚马逊近7天的销售额", teams, query_plan._normalize_component_value
    )
    assert hits == []


def test_reverse_lookup_keeps_business_specific_base():
    """主段是业务专名时主段匹配必须保留——这是反查存在的理由。

    「傲彼瑞」不在任何槽位词表里，应继续命中三个地区渠道并转澄清。
    """
    channels = ["傲彼瑞-美国", "傲彼瑞-加拿大", "傲彼瑞-tiktok", "莱福特-美国"]
    hits = query_plan._reverse_lookup_component_values(
        "查傲彼瑞的所有ASIN", channels, query_plan._normalize_component_value
    )
    assert set(hits) == {"傲彼瑞-美国", "傲彼瑞-加拿大", "傲彼瑞-tiktok"}


def test_generic_slot_terms_covers_platform_not_business_name():
    """排除集必须来自 intent_rules.json 现有词表，且区分通用词与业务专名。"""
    generic = query_plan._generic_slot_terms()
    assert query_plan._normalize_component_value("亚马逊") in generic
    assert query_plan._normalize_component_value("傲彼瑞") not in generic
