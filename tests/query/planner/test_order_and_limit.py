...
内核半边，内核 orderBy 形态为 {field, desc}（desc 为布尔，true 降序）。
"""
...
from opscli.query.services.planner import evidence_contract, query_plan
...
def _direction_of(entry: dict) -> str:
    """把内核 orderBy 的布尔 desc 归一为 DESC/ASC，便于断言可读。"""
...
def test_top_n_with_single_metric_resolves():
    """「取前10名」+ 唯一指标 → 按该指标降序取 10 行（Top N 的常识语义）。"""
    order_by, limit, unresolved = query_plan._resolve_order_and_limit(
...
def test_explicit_field_and_direction_resolve():
...
def test_ascending_wording_flips_direction():
    """升序词优先于 Top N 的默认降序。"""
    order_by, _limit, _unresolved = query_plan._resolve_order_and_limit(
        "发货数据集近7天按销售额升序排列，前5名", _fields(), _fields("销售额")
    )
    assert order_by and _direction_of(order_by[0]) == "ASC"


def test_order_without_limit_still_applies():
...
def test_ambiguous_metric_refuses_silently_applying_limit():
...
def test_time_expressions_are_not_row_limits(query: str):
...
def test_groupby_wording_is_not_a_sort_field():
