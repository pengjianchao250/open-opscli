...
def test_install_ops_amazon_rufus_outputs_login_guidance(monkeypatch):
    ...
    assert any("remote-consent status <country>" in step for step in payload["data"]["next_steps"])
    assert any("login-status <country>" in step for step in payload["data"]["next_steps"])
    assert any("没有可用登录态" in step for step in payload["data"]["next_steps"])
    assert any("opscli amazon-rufus watch-login <asin> <country>" in step for step in payload["data"]["next_steps"])
    assert any("--close-browser" in step for step in payload["data"]["next_steps"])
    assert any("amazon_rufus_get" in step for step in payload["data"]["next_steps"])
    assert any("opscli amazon-rufus get-backend <asin> <country>" in step for step in payload["data"]["next_steps"])
...
def test_interactive_install_ops_amazon_rufus_outputs_login_guidance(monkeypatch):
    ...
    assert any("remote-consent status <country>" in step for step in installed["next_steps"])
    assert any("login-status <country>" in step for step in installed["next_steps"])
    assert any("没有可用登录态" in step for step in installed["next_steps"])
    assert any("opscli amazon-rufus watch-login <asin> <country>" in step for step in installed["next_steps"])
    assert any("--close-browser" in step for step in installed["next_steps"])
    assert any("amazon_rufus_get" in step for step in installed["next_steps"])
    assert any("opscli amazon-rufus get-backend <asin> <country>" in step for step in installed["next_steps"])
