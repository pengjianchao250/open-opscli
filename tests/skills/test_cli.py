...
def test_install_ops_amazon_rufus_outputs_login_guidance(monkeypatch):
    ...
    assert any("auth_is_authenticated" in step for step in payload["data"]["next_steps"])
    assert any("auth_mcp_login" in step for step in payload["data"]["next_steps"])
    assert any("amazon_rufus_remote_consent_status" in step for step in payload["data"]["next_steps"])
    assert any("amazon_rufus_login_status" in step for step in payload["data"]["next_steps"])
    assert any("没有可用亚马逊 Rufus 登录态" in step for step in payload["data"]["next_steps"])
    assert any("amazon_rufus_watch_login" in step for step in payload["data"]["next_steps"])
    assert any("close_browser=true" in step for step in payload["data"]["next_steps"])
    assert any("amazon_rufus_get" in step for step in payload["data"]["next_steps"])
    assert any("MCP Tool 不可用" in step and "CLI fallback" in step for step in payload["data"]["next_steps"])
    assert any("拒绝" in step and "opscli CLI" in step for step in payload["data"]["next_steps"])
    assert any("RUFUS_PLATFORM_COOKIE_AUTH_ERROR" in step and "amazon_rufus_watch_login" in step for step in payload["data"]["next_steps"])
    assert any("只有上述两种情况" in step and "CLI fallback" in step for step in payload["data"]["next_steps"])
    assert any("不建议绑定信用卡" in step for step in payload["data"]["next_steps"])
    assert any("opscli amazon-rufus login-status" in step for step in payload["data"]["next_steps"])
    assert any("opscli amazon-rufus get-backend" in step for step in payload["data"]["next_steps"])
    ...
    assert not any("任意" in step and "CLI" in step for step in payload["data"]["next_steps"])
...
def test_interactive_install_ops_amazon_rufus_outputs_login_guidance(monkeypatch):
    ...
    assert any("auth_is_authenticated" in step for step in installed["next_steps"])
    assert any("auth_mcp_login" in step for step in installed["next_steps"])
    assert any("amazon_rufus_remote_consent_status" in step for step in installed["next_steps"])
    assert any("amazon_rufus_login_status" in step for step in installed["next_steps"])
    assert any("没有可用亚马逊 Rufus 登录态" in step for step in installed["next_steps"])
    assert any("amazon_rufus_watch_login" in step for step in installed["next_steps"])
    assert any("close_browser=true" in step for step in installed["next_steps"])
    assert any("amazon_rufus_get" in step for step in installed["next_steps"])
    assert any("MCP Tool 不可用" in step and "CLI fallback" in step for step in installed["next_steps"])
    assert any("拒绝" in step and "opscli CLI" in step for step in installed["next_steps"])
    assert any("RUFUS_PLATFORM_COOKIE_AUTH_ERROR" in step and "amazon_rufus_watch_login" in step for step in installed["next_steps"])
    assert any("只有上述两种情况" in step and "CLI fallback" in step for step in installed["next_steps"])
    assert any("不建议绑定信用卡" in step for step in installed["next_steps"])
    assert any("opscli amazon-rufus login-status" in step for step in installed["next_steps"])
    assert any("opscli amazon-rufus get-backend" in step for step in installed["next_steps"])
    ...
    assert not any("任意" in step and "CLI" in step for step in installed["next_steps"])
