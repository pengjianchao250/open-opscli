...
### 【铁律18】代码修改后必须更新变更记录文件

每次通过 Edit/Write 工具修改代码文件后，**必须立即**将改动记录追加到 `/Users/mask/python3/opscli/docs/change-log-pending.md`。
...
**变更原因**：为什么要改（1-2 句话）
**改动点**：改了哪些文件/模块，具体改了什么
**验证结果**：如何验证（测试命令/脚本运行结果）
**影响范围**：哪些功能受影响
**回滚方式**：如何回滚
---
```
...
### 【铁律19】编码前先思考，不假设、不掩盖困惑

在动手写代码之前，必须先完成以下检查：

- **显式声明假设**：对任何不确定的假设必须明确写出。如果不确定，先问用户
- **多方案时要列出**：如果存在多种理解或实现方式，必须全部列出让用户选择，禁止静默取一种
- **更简方案优先**：如果存在更简单的方案，必须主动提出。有理有据地反驳不合理的需求
- **有疑问就停下**：如果某个需求不清楚或令人困惑，立即停下来命名困惑点并提问，禁止猜测

**权衡**：这条铁律偏向谨慎而非速度。对于真正零歧义的小任务，可以适当简化。

### 【铁律20】极简优先，只写解决问题所需的最少代码

- **禁止超范围功能**：不实现用户没有要求的功能
- **禁止单次抽象**：如果代码只使用一次，不做抽象
- **禁止未请求的灵活性和可配置性**：不加用户没要求的参数、配置项或钩子
- **禁止不可能场景的错误处理**：不为逻辑上不可能发生的场景写防御代码
- **200 行能写 50 行的必须重写**：如果代码膨胀，问自己"资深工程师会认为这段代码过度复杂吗？"如果会，精简

### 【铁律21】精确变更，只改必须改的

编辑现有代码时：

- **不美化邻里代码**：不改adjacent代码的注释、格式或风格
- **不改不坏的东西**：不重构正常运行的代码
- **匹配现有风格**：即使自己会用不同风格，也必须匹配已有代码风格
- **发现无关死代码只报告不删除**：如果注意到无关的死代码，在注释中提及或口头上报告，不主动删除

当你的修改产生了孤立代码：

- **必须清理自己制造的孤儿**：删除因你的修改而变为未使用的 import、变量、函数
- **不删预存的死代码**：除非用户明确要求

**判断标准**：每一行改动都必须能追溯到用户的明确请求。

### 【铁律22】目标导向执行，定义成功标准并验证闭环
...
**强的验收标准**让你可以独立闭环。**弱的标准**（如"让它能跑"）则需要反复确认。

### 【铁律23】终端输出字符必须 GBK 兼容（Windows 控制台安全）

CLI 命令和 Skill 脚本中所有通过 `console.print()` / `typer.echo()` / `print()` 输出到终端的字符，**必须兼容 GBK 编码**（Windows PowerShell 默认编码）。
...
> **这些铁律有效的标志**：diff 中不必要的减少、因过度复杂导致的重写减少、澄清问题在实现之前而非犯错之后。
...
- **编号部门解析必须泛化**：规划器处理编号部门时，必须统一支持多位中文数字、阿拉伯数字及“项目”前缀，不得只修复单个示例；完整部门词必须优先于其他组件的子串匹配，并至少覆盖“十二部”“项目十一部”“22部”“项目二十二部”等同类表达；销售小组枚举采用“一部-B组”等带后缀名称时，也不得拿其“一部”主段去匹配完整部门词中的子串。
- **组件筛选必须保留语义极性**：部门、国家、渠道、品牌等枚举组件完成等值授权校验后，仍须按值所在语段区分包含与排除；排除单值用 `!=`、多值用 `not_in`，禁止因枚举命中而统一改写成 `=`/`in`。长枚举值不得连带命中其短前缀，同一值正负冲突时必须 fail-closed 澄清。
- **点名指标集合必须完整**：用户点名多个指标或使用稳定别名时，规划器必须逐项核对并完整投影；物理字段名不同但唯一中文口径相同可视为覆盖，缺失任一指标则转 `metric_not_in_dataset`，不得因为已命中部分指标就执行残缺模板。
