...
function cellHtml(value, rowIndex, key) {
  if (isRecord(value) || Array.isArray(value)) {
    ...
    return `<details class="cell-detail"><summary><span class="type-pill">${escapeHtml(label)}</span><span class="cell-detail-action">查看</span></summary><pre>${escapeHtml(JSON.stringify(value, null, 2))}</pre></details>`;
  }
  if (value === undefined) return '<span class="muted">—</span>';
  if (typeof value === "boolean") return `<span class="type-pill">${value ? "true" : "false"}</span>`;
...
function renderTable(rows, filter, sortKey, sortDir, primitiveKey, page, pageSize) {
  ...
  const head = keys.map((key) => `<th><button class="button button-quiet btn btn-link" type="button" data-sort-key="${escapeHtml(key)}">${escapeHtml(key)} ↕</button></th>`).join("");
  const table = `<div class="table-scroll"><table><thead><tr><th>#</th>${head}</tr></thead><tbody>${pageRows.map(({ row, index }) => `<tr><td class="tiny">${index + 1}</td>${keys.map((key) => `<td>${cellHtml(tableValue(row, key, primitiveKey), index, key)}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`;
  if (pageCount === 1) return table;
  return `${table}<div class="table-pagination"><span>第 ${currentPage.toLocaleString("zh-CN")} / ${pageCount.toLocaleString("zh-CN")} 页 · ${visibleCount.toLocaleString("zh-CN")} 条</span><div><button class="btn btn-outline-secondary btn-sm" type="button" data-table-page="-1" ${currentPage === 1 ? "disabled" : ""}>上一页</button><button class="btn btn-outline-secondary btn-sm" type="button" data-table-page="1" ${currentPage === pageCount ? "disabled" : ""}>下一页</button></div></div>`;
}
...
class JsonLensApp extends HTMLElement {
  ...
  renderField(field, required = false) {
    ...
    if (field.type === "checkbox") {
      return `<label class="check-field form-check"><input class="form-check-input" type="checkbox" data-param="${escapeHtml(field.key)}" ${value ? "checked" : ""}><span>${escapeHtml(field.label)}${requiredMark}</span></label>`;
    }
    if (field.type === "select") {
      return `<label>${escapeHtml(field.label)}${requiredMark}<select class="form-select" data-param="${escapeHtml(field.key)}">${field.options.map(([option, label]) => `<option value="${escapeHtml(option)}" ${String(value) === String(option) ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}</select></label>`;
    }
    if (field.type === "json") {
      return `<label class="field-wide">${escapeHtml(field.label)}${requiredMark}<textarea class="form-control" data-param="${escapeHtml(field.key)}" spellcheck="false" placeholder="${escapeHtml(field.placeholder || "")}">${escapeHtml(value)}</textarea></label>`;
    }
    ...
    return `<label>${escapeHtml(field.label)}${requiredMark}<input class="form-control" type="${field.type === "csv" ? "text" : field.type || "text"}" data-param="${escapeHtml(field.key)}" value="${escapeHtml(value)}" ${attrs}></label>`;
  }
  ...
  renderScenarioForm() {
    ...
    const identifierToggle = this.state.scenario === "product" ? `<div class="identifier-toggle"><span class="tiny">输入类型</span><div class="segmented">${[["asin", "ASIN"], ["code", "UPC/EAN/ISBN"]].map(([value, label]) => `<button type="button" class="segment ${this.state.params.identifierType === value ? "active" : ""}" data-identifier-type="${value}">${label}</button>`).join("")}</div></div>` : "";
    return `<div class="scenario-intro"><div><h3>${escapeHtml(definition.title)}</h3><p>${escapeHtml(definition.description)}</p></div><span class="endpoint-chip">${escapeHtml(this.state.scenario)}</span></div>
      ${identifierToggle}
      ${definition.selectionFields ? '<p class="field-hint">至少填写一项筛选条件；同一字段多个值默认匹配任一项，勾选项只会在开启时提交。</p>' : ""}
      ...
      ${optional.length ? `<details class="advanced-options" ${this.state.advancedOpen ? "open" : ""}><summary><span>高级参数</span><span class="tiny">${optional.length} 项可选</span></summary><div class="field-grid">${optional.map((field) => this.renderField(field)).join("")}</div></details>` : ""}`;
  }

  renderScenarioSelector() {
    return `<label class="scenario-selector">查询场景<select class="form-select" data-field="scenario" aria-label="查询场景">${Object.entries(SCENARIOS).map(([key, item]) => `<option value="${escapeHtml(key)}" ${key === this.state.scenario ? "selected" : ""}>${escapeHtml(item.title)}</option>`).join("")}</select></label>`;
  }

  renderRequestForm(compact = false) {
    return `<form class="stack" data-action="request">
      ...
      <details class="connection-options"><summary>连接设置 <span class="tiny">API 地址与鉴权</span></summary><div class="stack connection-body"><label>API Key <input class="form-control" type="password" data-field="apiKey" value="${escapeHtml(this.state.apiKey)}" placeholder="支持纯 Key 或 Bearer Key" autocomplete="off"></label><label>API 地址 <input class="form-control" data-field="endpoint" value="${escapeHtml(this.state.endpoint)}" aria-label="接口地址"></label></div></details>
      <details class="common-options"><summary>执行设置 <span class="tiny">等待策略</span></summary><div class="field-grid"><label class="check-field"><input type="checkbox" data-field="wait" ${this.state.wait ? "checked" : ""}><span>等待任务完成</span></label></div></details>
      <div class="row"><button class="button button-primary btn btn-success" type="submit" ${this.state.loading ? "disabled" : ""}>${this.state.loading ? "请求中..." : "运行" + escapeHtml(SCENARIOS[this.state.scenario].title)}</button><button class="button btn btn-outline-secondary" type="button" data-sample>载入样例</button></div>
      <div class="status-line" data-tone="${this.state.tone}">${escapeHtml(this.state.status)}</div>
    </form>`;
  }
  ...
  renderSummary() {
    ...
    return `<div class="summary-strip"><div class="metric"><span class="tiny">结构</span><strong>${shape}</strong></div><div class="metric"><span class="tiny">记录</span><strong>${rows ? rows.length : "—"}</strong></div><div class="metric"><span class="tiny">字段</span><strong>${keys}</strong></div><div class="metric"><span class="tiny">视图</span><strong>${this.state.view}</strong></div></div>`;
  }
  ...
  renderResultPanel() {
    ...
    return `<section class="panel card result-panel"><div class="panel-header card-header"><div><p class="eyebrow">adaptive result</p><h2>查询结果</h2></div><div class="result-toolbar"><input class="form-control" data-filter placeholder="筛选当前结果" value="${escapeHtml(this.state.filter)}"><div class="view-tabs nav nav-pills" role="tablist">${["table", "tree", "raw"].map((view) => `<button class="view-tab nav-link" type="button" data-view="${view}" aria-selected="${this.state.view === view}">${view === "table" ? "表格" : view === "tree" ? "树形" : "原始"}</button>`).join("")}</div>${downloadable ? '<button class="button btn btn-outline-secondary download-table" type="button" data-download-table title="下载当前筛选和排序后的 CSV 表格">下载 CSV</button>' : ""}</div></div>${this.renderSummary()}${content}</section>`;
  }
  ...
  render() {
    ...
    let layout;
    if (v === "a") {
      layout = `<div class="variant-layout variant-a"><aside class="panel card request-panel"><div class="panel-header card-header"><div><p class="eyebrow">scenario & filters</p><h2>查询场景</h2></div><span class="type-pill">prototype</span></div><div class="panel-body card-body">${this.renderRequestForm()}</div></aside>${this.renderResultPanel()}</div>`;
    } else if (v === "b") {
      layout = `<div class="variant-layout variant-b"><section class="hero-strip"><div><p class="eyebrow">result-first workspace</p><h2>先看数据，再决定下一步。</h2><p>适合运营场景：查询配置收进顶部工具栏，结果占据主区域。</p></div><div class="hero-actions"><button class="button btn btn-outline-secondary" type="button" data-sample>样例数据</button><button class="button button-primary btn btn-success" type="button" data-view="table">打开表格</button></div></section><section class="panel card"><div class="panel-body card-body">${this.renderRequestForm(true)}</div></section>${this.renderResultPanel()}</div>`;
    } else {
      ...
      layout = `<div class="variant-layout variant-c"><section class="panel card result-panel"><div class="panel-header card-header"><div><p class="eyebrow">inspector view</p><h2>数据检查器</h2></div><div class="result-toolbar"><button class="button btn btn-outline-secondary" type="button" data-view="tree">展开树</button><button class="button btn btn-outline-secondary" type="button" data-view="raw">原始 JSON</button></div></div>${this.renderSummary()}<div class="panel-body card-body">${this.state.view === "tree" ? renderTree(this.state.data) : this.state.view === "raw" ? `<pre class="raw-json">${escapeHtml(JSON.stringify(this.state.data, null, 2))}</pre>` : (sourceRows ? renderTable(sourceRows, this.state.filter, this.state.sortKey, this.state.sortDir, primitiveKey, this.state.tablePage, this.state.tablePageSize) : renderTree(this.state.data))}</div></section><aside class="stack"><section class="panel card"><div class="panel-header card-header"><div><p class="eyebrow">scenario & filters</p><h2>查询场景</h2></div></div><div class="panel-body card-body">${this.renderRequestForm(true)}</div></section><section class="panel card"><div class="panel-header card-header"><h2>顶层字段</h2></div><div class="panel-body card-body"><ul class="key-list">${keys.slice(0, 18).map((key) => `<li><span>${escapeHtml(key)}</span><strong>${escapeHtml(fieldType(key))}</strong></li>`).join("") || '<li>暂无字段</li>'}</ul></div></section></aside></div>`;
    }
    this.innerHTML = `<div class="shell"><header class="topbar"><div class="brand"><div class="brand-mark">{ }</div><div><p class="eyebrow">HTML-first API explorer</p><h1>JSON Lens <span class="type-pill">prototype</span></h1></div></div><div class="header-note">动态 JSON 结果 · 原生 Web Components<br>Variant ${v.toUpperCase()} · ${VARIANTS[v]}</div></header><main class="workspace">${layout}</main><nav class="prototype-switcher" aria-label="原型布局切换"><button class="switcher-button" type="button" data-variant-direction="-1" title="上一个布局">←</button><span class="switcher-label">${v.toUpperCase()} · ${VARIANTS[v]}</span><button class="switcher-button" type="button" data-variant-direction="1" title="下一个布局">→</button></nav></div>`;
  }
}
