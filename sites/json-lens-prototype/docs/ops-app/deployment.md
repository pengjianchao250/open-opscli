...
- `.opscli/app.json` 存在但未被 Git 跟踪，binding 指向 `test-keepa`，且 `app.yaml` 声明 `app_id: 1kKLO`、`title: test-keepa`。
...
平台路由、middleware 和 service 名称使用 `APPHUB_RESOURCE_ID` 隔离；`compose.apphub.yaml` 必须与统一模板保持一致，不在业务项目内自行修改。
