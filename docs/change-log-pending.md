...
**改动点**：新增 `api_credentials` 模块的 Provider 1:N Account 1:N Credential 模型、独立 MySQL 配置、AES-256-GCM 信封加密、v1 表结构、优先级加 LRU 并发领取、密钥版本 fencing、审计和统一 CLI；账号新增、轮换、启停和逻辑删除均通过 DML 直接读写 MySQL，删除后保留密钥历史与审计；表结构初始化只校验 MySQL，不要求尚未使用的 API 主密钥；Google Trends/SerpAPI、Canopy MCP、scrape.do 生产调用均改为凭据池取号，401/403 可在同一请求切换账号，SerpAPI 保留一次性 SQLite 迁移入口；本地 `D:\Gitlab\start-mcp.ps1 -InitializeSchema` 同时初始化共享采集表和 API 凭据池表。Provider 范围仅包含 `serpapi`、`canopy`、`scrape_do`，未修改 Keepa、卖家精灵和既有 `integration_accounts.py`。
**验证结果**：`tests/api_credentials` 18 项、`tests/google_trends` 85 项、`tests/scrape_do` 25 项、`tests/beta/canopy + tests/mcp/test_beta_tools.py` 40 项全部通过，共 168 项；`python -m compileall -q opscli tests` 与 `git diff --check` 通过。全量 `tests/` 仍被仓库既有基线阻断：Shopify 测试导入缺失 `_shopify_manager`、部分 Skills 测试在 importlib 模式下无法导入 bundled scripts、既有 Canopy 测试期待已注释的 `canopy-debug` 注册，并可能触发 pytest capture 清理异常；这些无关问题未纳入本次修改。
**影响范围**：Google Trends 的 SerpAPI 默认存储从 SQLite 切换为 MySQL；Canopy MCP 不再接受调用方直传 Key；scrape.do 不再读取环境变量、本地 token 文件或旧集成账号。部署前必须初始化表结构并注入独立 MySQL 连接和 Base64 主密钥。
