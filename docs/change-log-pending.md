...
**变更原因**：AppHub 已提供完整模板项目，脚手架、目录结构、具体开发命令和数据库实现应由模板项目维护；前后端最低开发要求、取数、平台、安全、迁移和源码交付限制仍需随 opscli 发版更新，不能全部移入创建后不再自动更新的业务项目。

**改动点**：`ops-app-build-spec` v0.0.4 将新项目流程改为克隆统一模板，并明确模板本地规范与当前 Skill 动态限制的优先级。删除 10 个历史 assets 和旧初始化说明；将原前后端长篇手册替换为两个简要 reference，只保留 UI 状态、统一请求、API 合同、分层、异步、事务和安全底线；继续保留真实数据访问、通用项目迁移、`opscli app create/init/push` 与线上部署边界。

**验证结果**：用项目 `.venv` 直接执行专项文件内 9 项合同检查，全部通过；`PYTHONUTF8=1` 下 Skill quick validate 通过；安装内容和已删除实现细节检查通过；测试文件编译与 `git diff --check` 通过。当前环境未安装 pytest，未新增依赖，因此没有通过 pytest runner 执行。

**影响范围**：仅影响 `ops-app-build-spec` 的安装内容、版本、清单描述和专项契约测试；不修改 `opscli app create/init/push`，不修改模板项目，不启动服务、不执行数据库迁移、不推送或部署。

**回滚方式**：还原本次 Skill、版本、清单、专项测试和本条变更记录。
...
## 2026-08-25 skills/mcp - Amazon 评论页面工具增加 MCP 静态规范入口

**变更原因**：`ops-amazon-reviews` 原先只描述 AI Chat 页面工具调用，未进入
Python/完整二进制产物，也没有可由 MCP 发现的规范入口。实际评论 Provider 依赖
...
- 全量发布清单校验仍命中既有问题：`ops-methods-card` 目录缺少 manifest 条目，
  与本次改动无关；本次评论 Skill 的产物矩阵已由聚焦测试覆盖。
