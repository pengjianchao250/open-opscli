...
**改动点**：利润相关成本字段统一默认 `0` 并取消必填；指定二区默认改为 `zone_1_2`（美东+美西）；`calculator detail` 改为只读展示 `allPlans` 全部方案、线路、费用值和区间，不提供方案切换；同步新品计算器 Skill 参考说明。
**验证结果**：按任务约束未新增测试用例，未运行 type-check、eslint 或格式化检查；完成代码差异自检和本地样例输出核对。
