...
**影响范围**：仅新增显式执行的历史迁移模块和 `scripts/migrate_seller_sprite_history.py`；不挂载公共 `opscli` 命令，不自动连接 MySQL，不改变在线 live 沉淀，也不会在未执行 purge 时删除源文件。生产入库前需用迁移账号初始化三张历史扩展表，并配置现有采集 MySQL 环境变量。
