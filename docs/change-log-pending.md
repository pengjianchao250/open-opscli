...
**变更原因**：需要按站点和单个 ASIN 查询实时竞价历史，并将列表页官方导出的完整竞价口径与详情页按广告类型展示的分页口径正确区分。
**改动点**：新增 `real-time-bidding` browser-route 场景和单 ASIN 校验；按更新时间倒序持续翻页并读取最新已完成历史任务，不创建新任务或消耗查询额度；分别读取该任务 SP、SB 第一页 100 条并按关键词严格合并，覆盖官方 46 列主表且排除 `Notes` 页；列表页摘要和详情页单广告类型响应不再被误当作完整导出数据。
**验证结果**：实时查竞价参数、历史任务选择、SP/SB 严格合并、46 列本地导出、官方 fixture 契约和 MCP 场景说明共 13 项定向回归通过；SellerSprite 与 MCP 回归（排除未改动且当前基线失败的 `test_cli_split.py`）487 项通过；`compileall`、JSON 解析和 `git diff --check` 通过。
**影响范围**：新增独立、只读的 `real-time-bidding` 场景；现有 SellerSprite 场景、任务创建、官方导出接口和额度策略不变。
**回滚方式**：回退实时查竞价场景注册、历史任务读取、SP/SB 合并、导出映射、测试、参考资料和本条记录。
---
