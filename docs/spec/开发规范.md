...
| `opscli.collector_monitor.config` | `opscli.seller_sprite.config` / `opscli.seller_sprite.services.account_bindings` | 复用 SellerSprite 队列和专属账号绑定数据库路径合同 |
| `opscli.collector_monitor.config` | `opscli.mcp.quota` | 复用 MCP quota SQLite 环境变量名，避免读写端路径漂移 |
...
`apphub_url` 只配置 AppHub 服务根地址，不包含 `/api/v1`。环境变量只使用
`OPSCLI_APPHUB_URL`，不设置同义变量，避免预发布和生产环境选择产生歧义。

建站模板使用完整 Git clone 地址 `app_template_repo` 和分支 `app_template_branch`，
对应环境变量固定为 `OPSCLI_APP_TEMPLATE_REPO`、`OPSCLI_APP_TEMPLATE_BRANCH`。
它们与 `OPSCLI_APPHUB_URL` 应在预发布或生产环境配置中成组维护，但不得根据 AppHub 域名
推导模板仓库域名。配置优先级与 OPS、Polaris 等现有配置保持一致，不新增特殊覆盖规则。

`opscli app` 不配置 Gitea 根地址或全局应用源码仓库。每个应用的完整 `repo_url`
必须由当前环境 AppHub 的 `POST /api/v1/apps` 或 `GET /api/v1/apps/{app_id}/git-config` 返回，
并包含协议、主机、端口、组织和仓库名；`app_id` 是大小写敏感的五位公开应用 ID，CLI 不得用 slug 代替应用身份，也不得自行拼接 `apps/{slug}`。
模板仓库与应用源码仓库相互独立。模板获取、建站过程、开发顺序和代码规范由
`ops-app-build-spec`、模板仓库和项目内规范负责，不属于 `opscli app` 职责。
`opscli app create` 只创建应用并保存基础信息，`init` 只初始化和绑定 Git，`push` 只推送
源码，`release` 确保源码已推送后再创建 AppHub 版本。用户不需要记忆严格顺序，命令内部
按需幂等补齐应用信息和 Git 初始化。

`init` 和 `push` 复用的仓库准备链路必须通过 `GET /api/v1/apps/{app_id}/env` 检查
`OPSCLI_MCP_REST_API_BASE_URL`。缺失或为空时，按 AppHub 控制面 origin 精确映射预发布
`https://mcp.ops.aukeyit.com` 或生产 `https://ops.mcp.xenkee.com`，合并完整已有自定义
`env` 后调用 PUT 保存；已有非空值不得自动覆盖，`platform_env` 不得回写。未知 AppHub
环境必须停止，不能猜测或回退到默认环境。
...
| `OPSCLI_APPHUB_URL` | 选择当前环境的 AppHub 控制面 |
| `OPSCLI_APP_TEMPLATE_REPO` | 选择当前环境的建站模板完整 Git 地址 |
| `OPSCLI_APP_TEMPLATE_BRANCH` | 选择建站模板分支，默认 `master` |
