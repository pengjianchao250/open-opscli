...
`apphub_url` 只配置 AppHub 服务根地址，不包含 `/api/apphub/v1`。环境变量只使用
...
`opscli app` 不配置 Gitea 根地址或全局站点源码仓库。每个站点的完整 `repo_url`
必须由当前环境 AppHub 的 `POST /apps` 或 `GET /apps/{slug}/git-config` 返回，
并包含协议、主机、端口、组织和仓库名；CLI 不得自行拼接 `apps/{slug}`。
模板仓库与站点源码仓库相互独立：模板仅用于全新项目初始化，拉取时必须禁用 Git
credential helper，避免把站点仓库 token 发送到模板仓库；已有源码不得被模板覆盖。
用户在 Codex 中表达新建站点、新建看板或从零开发运营数据应用意图时，Codex 必须加载
`ops-app-build-spec` 作为统一入口；Skill 阶段 0 编排 `opscli app create → opscli app init`，
模板成功后同一个 Skill 才进入正式开发，最终执行 `opscli app push`。模板完成前不得生成
assessment、README、前后端、应用配置或部署文件。
...
| `OPSCLI_APP_TEMPLATE_BRANCH` | 选择建站模板分支，默认 `main` |
