...
当前处理 Collector MCP 中 SellerSprite 和 Keepa 新产生的成功任务。历史生产任务迁移仅保留 TODO。
...
后台 Worker 只依赖 `source_system -> Parser -> Repository`，不包含来源场景分支。来源模块只需注册自己的 Submitter、Parser，并在存在独立成功状态库时注册 Reconciler。
...
不得从文件名、目录或主机名推断。Outbox 首次启用时固定 `live_cutover_at`。SellerSprite 在任务成功事务内追加独立自增成功事件，对账游标按成功事件顺序推进，不依赖可能乱序完成的任务创建 ID；自动对账只覆盖 cutover 之后的新成功任务。Keepa 同步成功路径在完整写入结果文件后直接提交 Outbox；Collector 公开入口强制使用配置的受控输出目录，Reconciler 周期扫描 cutover 后且尚未存在于 Outbox 的 `result.json`，恢复结果落盘与 Outbox 提交之间的进程崩溃窗口，不扫描或回填 cutover 前的历史目录。
