...
7. 发起 Rufus 获取前先调用 `amazon_rufus_login_status(country)`；没有可用亚马逊 Rufus 登录态时再调用 `amazon_rufus_watch_login(asin, country, close_browser=true)`。
8. OPS 平台 Cookie 鉴权错误、`RUFUS_PLATFORM_COOKIE_AUTH_ERROR` 或 401 时，调用 `amazon_rufus_watch_login(asin, country, close_browser=true)`；本分支不允许 CLI fallback。
...
9. 如果亚马逊 Rufus 登录态缺失，调用 `amazon_rufus_watch_login(asin, country, close_browser=true)` 完成登录采集，再次检查登录态。
10. 如果返回 OPS 平台 Cookie 鉴权错误、`RUFUS_PLATFORM_COOKIE_AUTH_ERROR` 或 401，调用 `amazon_rufus_watch_login(asin, country, close_browser=true)`，采集后按原问题来源调用 `amazon_rufus_get`。
11. 调用 `amazon_rufus_get` 获取 Rufus 回答并写入报告。
12. 如果命中 `RUFUS_SECRET_NOT_READY`、`RUFUS_HEADLESS_CAPTURE_ERROR` 或 `RUFUS_HEADLESS_REQUEST_ERROR`，本次 Skill 调用尚未触发登录恢复时，按 `amazon_rufus_logout -> amazon_rufus_watch_login -> amazon_rufus_get` 恢复一次。
13. 每次 Skill 调用最多触发一次登录恢复；恢复后仍失败时直接报错，不再重复打开登录窗口。
14. 其他错误不允许 CLI fallback。
