...
description: 使用 CLI 模式查询本地数据集索引并执行 库存健康监控器 数据查询
version: v0.1.0
---

# ops-inventory-health-monitor (CLI 模式)
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/inventory-health-monitor_query1.json \
  --run --pretty
```
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/inventory-health-monitor_query2.json \
  --run --pretty
```
...
```bash
opscli skills install ops-inventory-health-monitor            # 安装
opscli skills install ops-inventory-health-monitor --force     # 强制重装
opscli skills status --pretty                                # 查看版本
opscli skills upgrade ops-inventory-health-monitor             # 升级
```
