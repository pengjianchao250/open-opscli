...
# ops-inventory-health-monitor (MCP 无状态模式)
...
result = query_build_and_run(
    dataset="库存周转数据（可售天数、平台库存、海外仓库存等）",
    dimensions=["asin"],
    limit=1000,
    session_id="xxx",
...
### `calculate_inventory_health_mcp.py` — 计算库存健康评级和风险识别（无 opscli 依赖）

从 stdin 读取 JSON 输入或通过 `--input` 指定文件，执行分析计算并输出 JSON。**不依赖 opscli 命令行工具**。

```bash
# 从 stdin 读取
echo '{"key": "value"}' | python scripts/calculate_inventory_health_mcp.py

# 从文件读取
python scripts/calculate_inventory_health_mcp.py --input /tmp/data.json --pretty
```

### `generate_replenishment_plan_mcp.py` — 生成补货数量与时机建议（无 opscli 依赖）

从 stdin 读取 JSON 输入或通过 `--input` 指定文件，执行分析计算并输出 JSON。**不依赖 opscli 命令行工具**。

```bash
# 从 stdin 读取
echo '{"key": "value"}' | python scripts/generate_replenishment_plan_mcp.py

# 从文件读取
python scripts/generate_replenishment_plan_mcp.py --input /tmp/data.json --pretty
```
...
```python
# 安装
skills_install(name="ops-inventory-health-monitor", skills_dir="/Users/mask/.config/opencode/skills")

# 强制重装
skills_install(name="ops-inventory-health-monitor", force=True, skills_dir="/Users/mask/.config/opencode/skills")

# 查看版本
skills_status(skills_dir="/Users/mask/.config/opencode/skills")

# 升级
skills_upgrade(name="ops-inventory-health-monitor", skills_dir="/Users/mask/.config/opencode/skills")
```
