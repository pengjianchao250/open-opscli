...
- `seller_sprite_listing_analysis_submit`：提交 Listing Analysis 页面查询任务，立即返回本地 `job_id`。
- `seller_sprite_listing_analysis_status`：按 `job_id` 续查本地提交状态和远端 AI 任务状态。
- `seller_sprite_listing_analysis_result`：按 `job_id` 读取 Listing Analysis 最终结果；未完成时返回 `ready=false`。
...
4. `listing-analysis` 必须走三段式：先 `seller_sprite_listing_analysis_submit`，等待约 3 分钟，再用 `seller_sprite_listing_analysis_status` / `seller_sprite_listing_analysis_result` 续查；不要让 `seller_sprite_run` 同步阻塞等待 `listing-analysis` 完整结果。
...
| `succeeded` | 本地提交已完成 | 普通任务返回结果；Listing Analysis 若只有 `task_id`，继续查远端 AI 结果 |
| `failed` | 后台执行失败 | 报告 `error.message`，不要复用旧导出文件 |

## Listing Analysis 三段式

1. 提交：`seller_sprite_listing_analysis_submit(asin, station="GLOBAL", site="US", export_format="json")`，返回 `job_id`。提交阶段由后端使用页面输入 ASIN + 点击查询按钮，不要让 Agent 自行传 `mode`。
2. 续查：约 3 分钟后调用 `seller_sprite_listing_analysis_status(job_id)`；如果 `ready=false`，只提示继续等待，不要重新 submit。
3. 取结果：调用 `seller_sprite_listing_analysis_result(job_id, export_format="json")`；`ready=false` 时稍后重查，`ready=true` 时返回 `row_count` 和 `export`。
