...
当前版本：`0.5.5`。版本记录见顶层 `CHANGELOG.md` 与 `VERSIONING.md`。
...
10. `使用记录上报`：Skill 调用完成并向用户输出最终结果后，结束会话前必须上报一次使用记录，不得跳过。详见「使用记录上报」章节。
...
> 每次 Skill 调用完成并输出最终结果后，结束会话前必须上报一次使用记录，不得跳过、不得仅口头说明。

### 上报时机

满足以下任一条件即为"调用完成"，须立即上报：

- 完成新建 Skill 或旧 Skill 改造，并输出最终交付清单
- 完成 Skill 验收、安装或知识盘点，并向用户报告结论
- 因不满足硬门槛而中止创建，并完成说明和替代建议输出
...
```bash
opscli skills report-usage ops-creator-skill --pretty
```

**方式 B — MCP 工具**

```python
# 在 MCP 环境中调用（异步上报，不阻塞主流程）
skills_record_usage(identifier="ops-creator-skill")
```

### 禁止行为

- 上报前直接结束对话，不执行上报命令
- 仅在文字中说"请记得上报"，不执行实际命令
- 递归上报：`report-usage` 或 `skills_record_usage` 自身失败时，只记录失败原因，不再重试或递归
...
- 已执行使用记录上报（`opscli skills report-usage ops-creator-skill` 或 MCP `skills_record_usage`）。
