...
17. 修改标题、局部样式和位置时使用三个显式目标工具；样式一次只改一个 `styleKey`，移动前读取当前布局，并按 Skill 的固定 12 列规则检查边界。
18. 默认组合必须通过一个 `dashboard_editor_batch_create_charts` 请求整体提交；不得拆成逐图新增、逐图选集或独立字段配置。
...
7. 为每张图表确定业务标题；默认组合的图表类型、顺序和布局直接遵循 `SKILL.md`，不从代码或其他合同补充规则。
8. 使用字段目录一次规划全部图表的完整 `fieldLists`，并在任何页面写入前完成图表字段规则校验。
9. 用一个 `dashboard_editor_batch_create_charts` 请求传入唯一 `datasetId` 和完整 `charts`；禁止调用旧的逐图新增、选集或独立批量配置链路。
...
- 工具 schema 由页面本轮注册结果提供，不再通过 `getContext` 重复返回。
...
3. 一次确定每张图表的 `viewType/title/layout/fieldLists`；默认组合布局遵循 `SKILL.md`，显式单图可按用户要求传入布局，未指定时使用页面默认尺寸。
4. 用一个 `dashboard_editor_batch_create_charts` 请求创建并配置全部图表，不降级为逐图写入。
...
用户明确指定图表类型时优先服从用户。未指定类型时，只选择 `SKILL.md` 定义的默认组合：

| 模板键                 | 识别线索                   | 类型与布局来源              |
| ---------------------- | -------------------------- | --------------------------- |
| `marketing_conversion` | 广告、流量、活动、营销、转化 | `SKILL.md` 营销/转化默认组合 |
| `supply_chain`         | 库存、物流、履约、供应链   | `SKILL.md` 供应链默认组合   |

无法唯一判断模板时询问用户；不创建默认组合外的兜底方案，也不在参考文档中复制类型和尺寸列表。

组合约束：

- 默认组合按 `SKILL.md` 的数量和顺序完整创建，不得缩减为单图或部分组合。
- 必须用真实字段目录验证可行性；缺少默认组合所需字段时停止并说明阻塞，不擅自替换类型。
- 全部图表共享一个 `datasetId`。任何图表需要第二数据集才能成立时，停止并调整组合，不得跨数据集拼接。
- 图表类型、数据集、字段必须在创建前确定；创建后只允许按已确认计划批量写入，不允许用页面结果反向试字段。

### 默认组合布局

画布固定为 12 列。精确图表类型、顺序和宽高由 `SKILL.md` 维护并作为批量创建参数提交；不读取用户输入的列数，也不另行计算尺寸。

创建规则：

1. 选择唯一匹配的模板键，并保持 `SKILL.md` 给出的图表数量和顺序。
2. 营销/转化模板保持两张摘要图同高、三张分析图全宽；供应链模板保持两张摘要图同高、两张分析图全宽。
3. `indicator` 只配置 1 个度量；`pie_circle` 只配置 1 个类别维度和 1 个度量；其他图表按各自字段规则配置。
4. 批量返回后逐张核验 `chartId/viewType/title/layout/fieldLists` 与本轮完整计划一致；任意矩形重叠都判定为 `FAIL`。
