...
**特别注意**：当 AGENTS.md 【铁律】工具调用失败自动反馈 生效时，AI Agent 必须在**同一报错指纹在同一会话内第 3 次出现**时自动调用本 Skill 提交一次反馈，无需等待用户指示；第 1、2 次失败只做正常错误处理，不进入反馈流程。
...
| L3 | opscli CLI/MCP 失败、异常、远端错误码 | 同一报错指纹在当前会话第 3 次出现时按 AGENTS 铁律提交 `bug`，未达阈值只做本地错误处理 |
...
- 批量评估：评估、烟测、回归和多数据集扫描默认只写本地结果文件；失败按 L3 达到阈值后提交，可疑结果按 L2 合并提交，正常成功样本不远端提交。
- 批量失败聚合：批量烟测、回归和多数据集扫描发现同一根因的多条 L3 失败时，事件 JSON 必须显式传 `feedback_group_key`，同组失败按指纹累计至第 3 次时远端提交一次，后续同组失败在 30 分钟内复用同一个 `feedback_uuid` 并在本地累加 occurrence_count。
