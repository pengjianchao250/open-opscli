...
description: 融合内部品类销售集中度、爬虫竞品数据、品牌搜索分析，应用波特五力 + 定位图 + 四行动框架，输出品类级竞争情报看板与策略建议。支持 CLI 模式和 MCP 模式。
version: v0.1.0
---

# 竞争情报分析师

融合内部品类销售集中度、爬虫竞品数据、品牌搜索分析，应用波特五力 + 定位图 + 四行动框架，输出品类级竞争情报看板与策略建议。 支持 CLI 模式和 MCP 无状态模式。
...
- Porter's Five Forces Scoring：基于 HHI 和市场数据计算五力评分
- Positioning Map Generation：生成价格-评分定位图数据
- Four Actions Framework Strategy：基于定位分析生成 Eliminate/Reduce/Raise/Create 策略
- Competitor Profiling：构建竞品画像
...
|--------|--------------|------|
| ds_d35ac6f3910c | `内部销售数据（销售额、ASIN 数量、毛利率等）` | 内部销售数据 |
| ds_pdTYjvLRCadv | `竞品 Listing 快照（价格、星级、评论数、排名）` | 竞品 Listing 快照 |
| ds_xsTOkHIpr3ad | `搜索词表现数据` | 搜索词表现 |
| ds_I13gHlcdwevS | `品牌搜索表现数据` | 品牌搜索表现 |
...
| 脚本 | 模式 | 说明 |
|------|------|------|
| `scripts/competitive_analysis.py` | CLI | 执行完整 3-Layer 竞争分析 |
| `scripts/competitive_analysis_mcp.py` | MCP | 执行完整 3-Layer 竞争分析（无 opscli 依赖） |
| `scripts/core.py` | 通用 | 竞争分析核心常量和函数 |
