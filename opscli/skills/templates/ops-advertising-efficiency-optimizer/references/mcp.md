...
mcp-version: v0.1.0
description: 使用 MCP Tool 查询广告效率数据并计算指标（无状态模式）
---

# ops-advertising-efficiency-optimizer (MCP 无状态模式)

使用 MCP Tool 查询广告运营数据，通过本地缓存索引辅助字段检索，使用 `scripts/analyze_ads_efficiency_mcp.py` 等脚本计算广告效率指标。**无状态模式**：服务器不保存用户 OAuth 凭证，所有认证信息由调用方传入。
...
- 广告效率分析通过 `scripts/analyze_ads_efficiency_mcp.py` 完成（无 opscli 依赖）
- ROAS/ACOS 计算通过 `scripts/calculate_roas_acos_mcp.py` 完成（无 opscli 依赖）
- 预算分配通过 `scripts/ads_budget_allocator_mcp.py` 完成（无 opscli 依赖）
...
campaigns = [
    {"campaign_name": "SP-Exact", "ad_group_name": "Exact", "ad_type": "SP",
     "cost": 5200, "sales": 22000, "clicks": 3500, "impressions": 120000, "conversions": 420},
    {"campaign_name": "SB-Brand", "ad_group_name": "Brand", "ad_type": "SB",
     "cost": 2100, "sales": 10000, "clicks": 1800, "impressions": 50000, "conversions": 180}
]

input_data = {"campaigns": campaigns, "benchmarks": None}

result = subprocess.run(
    ["python", "scripts/analyze_ads_efficiency_mcp.py"],
...
### `analyze_ads_efficiency_mcp.py` — 广告效率诊断

从 stdin 读取 JSON 输入或通过 `--input` 指定文件，分析广告活动效率并输出 JSON。**不依赖 opscli 命令行工具**。

**用法：**

```bash
# 从 stdin 读取
echo '{"campaigns": [...], "benchmarks": {...}}' | python scripts/analyze_ads_efficiency_mcp.py

# 从文件读取
python scripts/analyze_ads_efficiency_mcp.py --input /tmp/ads_data.json --pretty

# 自定义基准
python scripts/analyze_ads_efficiency_mcp.py --input /tmp/ads_data.json \
  --benchmarks '{"acos_target": 0.18, "roas_target": 5.5, "cpc_target": 1.2, "ctr_target": 0.004}' \
  --pretty
```

### `calculate_roas_acos_mcp.py` — ROAS/ACOS 快速计算

从 stdin 读取 JSON，计算 ROAS、ACOS、CPC、CTR、CVR 指标。

```bash
echo '{"cost": 1000, "sales": 5000}' | python scripts/calculate_roas_acos_mcp.py
```

### `ads_budget_allocator_mcp.py` — 预算重新分配

从 stdin 读取 JSON，基于 ROAS 重新分配广告预算。

```bash
echo '{"campaigns": [...], "total_budget": 10000}' | python scripts/ads_budget_allocator_mcp.py
```
...
```python
# 安装
skills_install(name="ops-advertising-efficiency-optimizer", skills_dir="/Users/mask/.config/opencode/skills")

# 强制重装
skills_install(name="ops-advertising-efficiency-optimizer", force=True, skills_dir="/Users/mask/.config/opencode/skills")

# 查看版本
skills_status(skills_dir="/Users/mask/.config/opencode/skills")

# 升级
skills_upgrade(name="ops-advertising-efficiency-optimizer", skills_dir="/Users/mask/.config/opencode/skills")
```