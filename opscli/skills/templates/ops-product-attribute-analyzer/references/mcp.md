...
# ops-product-attribute-analyzer (MCP 无状态模式)
...
### `calculate_weighted_share_mcp.py` — 计算销量加权市场份额（无 opscli 依赖）

从 stdin 读取 JSON 输入或通过 `--input` 指定文件，执行分析计算并输出 JSON。**不依赖 opscli 命令行工具**。

```bash
# 从 stdin 读取
echo '{"key": "value"}' | python scripts/calculate_weighted_share_mcp.py

# 从文件读取
python scripts/calculate_weighted_share_mcp.py --input /tmp/data.json --pretty
```

### `analyze_attribute_combo_mcp.py` — 分析属性组合机会（无 opscli 依赖）

从 stdin 读取 JSON 输入或通过 `--input` 指定文件，执行分析计算并输出 JSON。**不依赖 opscli 命令行工具**。

```bash
# 从 stdin 读取
echo '{"key": "value"}' | python scripts/analyze_attribute_combo_mcp.py

# 从文件读取
python scripts/analyze_attribute_combo_mcp.py --input /tmp/data.json --pretty
```

### `generate_market_portrait_mcp.py` — 生成市场画像（无 opscli 依赖）

从 stdin 读取 JSON 输入或通过 `--input` 指定文件，执行分析计算并输出 JSON。**不依赖 opscli 命令行工具**。

```bash
# 从 stdin 读取
echo '{"key": "value"}' | python scripts/generate_market_portrait_mcp.py

# 从文件读取
python scripts/generate_market_portrait_mcp.py --input /tmp/data.json --pretty
```
...
```python
# 安装
skills_install(name="ops-product-attribute-analyzer", skills_dir="/Users/mask/.config/opencode/skills")

# 强制重装
skills_install(name="ops-product-attribute-analyzer", force=True, skills_dir="/Users/mask/.config/opencode/skills")

# 查看版本
skills_status(skills_dir="/Users/mask/.config/opencode/skills")

# 升级
skills_upgrade(name="ops-product-attribute-analyzer", skills_dir="/Users/mask/.config/opencode/skills")
```
