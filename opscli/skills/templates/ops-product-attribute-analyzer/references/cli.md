...
description: 使用 CLI 模式查询本地数据集索引并执行 产品属性分析器 数据查询
version: v0.1.0
---

# ops-product-attribute-analyzer (CLI 模式)
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/product-attribute-analyzer_query1.json \
  --run --pretty
```
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/product-attribute-analyzer_query2.json \
  --run --pretty
```
...
```bash
opscli skills install ops-product-attribute-analyzer            # 安装
opscli skills install ops-product-attribute-analyzer --force     # 强制重装
opscli skills status --pretty                                # 查看版本
opscli skills upgrade ops-product-attribute-analyzer             # 升级
```
