...
`assets/backend/` 提供项目级 `CLAUDE.md`、`AGENTS.md` 模板与 `docs/开发指南/` 全文规范，编写后端代码需要完整论述或代码模板时参考；其中 uv、Alembic、Compose 与 `/health`、`/ready` 探针的描述与 AppHub 运行合同冲突时，以 `backend-standard.md` 与 `deployment-standard.md` 为准。
