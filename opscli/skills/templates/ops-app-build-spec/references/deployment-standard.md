...
- `ops-app.config` 在依赖安装后复制，使项目 ID 或应用名变化只触发构建层，不重装依赖。
...
- Nginx runtime 必须创建并赋予非 root 用户权限给 `/tmp`、`/var/cache/nginx`、`/var/run`；启动参数将 pid 放在 `/tmp/nginx.pid`，不得依赖 `/var/run/nginx.pid` 的 root 写权限。访问日志和错误日志写入 stdout/stderr。
...
- 一个命名数据卷，用于 `/data`。
- 前后端服务都必须声明 `image`。镜像仓库名按服务区分：`<projectId>-<appName>-frontend` 和 `<projectId>-<appName>-backend`；每个仓库同时发布 `:sha-<git-sha>` 与 `:latest` 两个 tag。示例：`1234-orders-web-frontend:sha-3f8a2c1`、`1234-orders-web-frontend:latest`。

服务间使用 Compose 服务名通信。只有浏览器需要访问的端口才映射到宿主机；后端只使用 `expose: 8000`，由 Nginx 通过 `http://backend:8000` 访问。
...
- `frontend` 映射 `127.0.0.1:${OPS_APP_PORT:-8080}:8080`；外层 Nginx 在容器网络内时改为 external network，不发布宿主端口。
...
- `read_only: true` 为默认运行策略；前端至少把 `/tmp`、`/var/cache/nginx`、`/var/run` 声明为 `tmpfs`，后端把 `/tmp` 声明为 `tmpfs`。镜像中的静态文件和 Python 代码不可写。
...
`appName` 与 `projectId` 必须来自根目录 `ops-app.config`。Vite、Nginx 渲染和 Compose 镜像名必须复用同一配置；禁止在 `.env`、Compose、Dockerfile 或其他文件维护互相独立的硬编码 ID。Compose 中允许出现由 Skill 渲染得到的镜像名，但它必须视为派生产物，每次配置变更都重新生成并校验。
