...
# ---------------------------------------------------------------------------
# 图表数据加载（MCP 模式：仅从文件读取）
# ---------------------------------------------------------------------------


def load_chart_data(input_path: str) -> tuple[list[dict], str | None, dict | None]:
    """加载通过 MCP query_chart 获取并保存的图表数据。
    ...
    if isinstance(content, dict) and "data" in content:
        data = content["data"]
        if "queries" in data:
            # chart run 输出格式（MCP query_chart run=True 的标准返回）
...
def load_dc_data(dc_input: str | None) -> list[dict] | None:
    """加载通过 MCP query_build_and_run 获取的 dataComparison 环比结果。
...
# ---------------------------------------------------------------------------
# 异常检测引擎（与 chart_analyze.py 完全一致）
# ---------------------------------------------------------------------------


def build_alias_map(queries: list[dict]) -> dict[str, dict]:
...
def detect_anomalies_trend(
    ...
    dc_aliases = [
        ...
        and not k.startswith("diff_")
        and not k.startswith("pct_")
    ]
...
def generate_findings(
    ...
    if profit_alias and revenue_alias:
        ...
        findings.append(
            f"整体毛利率 {margin:.2f}%（总原价 {total_revenue:,.0f}，总毛利 {total_profit:,.0f}）"
        )
    ...
    if critical_count > 0:
        findings.append(
            f"发现 {critical_count} 个严重异常（{warning_count} 警告，{info_count} 提示）"
        )
...
def main() -> None:
    ...
    # 加载图表数据（仅从文件）
    queries, chart_uuid, _merged = load_chart_data(args.input)
