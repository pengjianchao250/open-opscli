...
**先分流，再决定要不要问**——币种有两类诉求，只有第二类才提问：

| 诉求 | 判断 | 动作 |
|------|------|------|
| 目标币种换算 | 用户说"用美元/按 USD/欧元口径/分别用人民币和加拿大元" | **不问**，直接传币种参数（MCP `global_currency=`、CLI `--global-currency`），多币种逐币种各查一次 |
| 原币字段口径 | 用户要"原币/站点币种"或做对账等精准口径，且同一指标存在 `xxx` 与 `xxx_cny` | **要问**，用 `AskUserQuestion` 确认选 CNY 字段还是原币字段 |

要求：
- 币种换算是请求参数，不是维度或筛选字段：元数据里没有 `currency` 字段属正常，不因此提问，也不因此判定"不支持按币种查询"。
- 币种意图仅支持 USD/GBP/CAD/EUR/JPY/CNY；未表达币种意图时不传该参数，由服务端回退用户默认币种，输出时按返回的 `meta.currency` 声明币种，禁止本地默认选 CNY 字段替代。
