...
`query_component` 只用于权限枚举，不是业务结果数据集；只有用户明确请求枚举/可用值（“有哪些渠道可选”“列出当前可见部门”以及短语级 `show/list/display available`、`what values are available for <字段>`、`what <字段> values are available`）时才可作为查询目标并生成可执行的维度查询模板，普通业务分析和 `available sales/inventory` 等业务表达仍须阻断。组件枚举的 Top N 只限制返回条数，不要求虚构排序指标。自然语言选表只依据当前账号元数据中的中文名称和中文说明；英文 key 仅在用户明确给出精确完整技术标识时精确匹配，不能从中文请求推断或模糊匹配。组件筛选值必须来自用户点名值与当前账号授权枚举的完整等值匹配；“按部门”“各事业部”“中查看部门”“by 事业部”“group by 部门”等分组或句法片段只表示字段/分组，不得当作具体筛选值。组件筛选允许“只看字段值”“字段值除外”等省略系词的自然表达；字段前置的“渠道 A、B”和字段后置的“A、B 渠道”都必须把完整列表归属同一字段，再与授权枚举求交集。整组排除极性必须跨 `、/逗号/和/与/或` 作用于全部列表成员，不能把分隔符后的值反转为包含。完整销售小组、渠道等复合值一旦命中，其内部部门、大组、品牌或国家片段不得再次消费。字段前置的显式标签优先于值内部或后续出现的后置标签。筛选表达中的字段标签只表示筛选，不得同时加入分组维度；“同时/又排除”必须进入极性冲突澄清。
