...
6. `query_component` 类数据集**只用于权限枚举**，不是业务结果数据集。明确请求枚举/可用值时才可作为查询目标并下发维度查询模板；普通业务分析仍返回 `business_dataset` 澄清。
7. 已确认的数据集完整名称只用于选表，不参与字段或组件值识别；完整授权字段标签的命中区间优先于其中的稳定短别名。分组句法（如“按部门”“各事业部”“中查看部门”“by 事业部”“group by 部门”）不得解析为组件筛选值。组件筛选须统一覆盖有无系词、前置排除、后置“除外”和多值连接表达；筛选字段标签不进入分组，完整复合值优先于其内部跨字段子串，同值正负冲突在下发模板前澄清。
