...
> **"近 N 天"默认包含今天**。用户未明确说明时按此理解，但必须在查询前参数摘要中明示该假设（"以下统计范围包含今天"），让用户有机会纠正。
...
| 本月前 N 天 vs 上月完整月 | **必须用 AskUserQuestion 澄清**：天数严重不对等，提供两方案：A) 用上月同期 N 天、B) 计算日均值 |
...
| **最近一个月** | 歧义（近30天 vs 当月？） | **必须用 AskUserQuestion 确认** |
| 本周 | 周一至今天 | 用户可能想要完整周 |
| 上周 | 上周一至上周日 | — |
| **最近一周** | 近7天滚动（≠"上周"） | 与"上周"含义不同，需用 AskUserQuestion 确认 |
| 上个月同期 | 上月 1 日~同日 | 需用 AskUserQuestion 确认具体日期范围 |
...
字段候选超过 4 个时，先文本展示完整候选，再用 AskUserQuestion 展示最相关的 2~3 个和"其他（输入字段名或序号）"。

### 常见歧义类型

| 歧义类型 | 检测方式 | 处理 |
|---------|---------|------|
| 同名多角色（人员/组织） | metadata 中 verbose_name 含"人员/小组/团队"且 ≥2 个 | 列出所有角色并用 AskUserQuestion 确认 |
| 组织层级歧义 | 存在多级组织维度（大组 → 小组 → 个人） | 用 AskUserQuestion 确认要查哪个层级 |
...
| **最近一个月** | 歧义 | **必须用 AskUserQuestion 确认** |
| 本周 | 周一至今天 | 用户可能想要完整周 |
| **最近一周** | 近7天（≠上周） | 与"上周"不同，需用 AskUserQuestion 确认 |
...
- **用户未指定库存类型时**：库存有 ≥3 个变体字段（可售/在途/海外仓/国内仓等），列出所有变体并用 AskUserQuestion 让用户选择，不得默认选"总库存"
