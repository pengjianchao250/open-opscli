...
description: 使用 CLI 模式查询本地数据集索引并执行 退款优先级矩阵 数据查询
version: v0.1.0
---

# ops-refund-priority-matrix (CLI 模式)
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/refund-priority-matrix_query1.json \
  --run --pretty
```
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/refund-priority-matrix_query2.json \
  --run --pretty
```
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/refund-priority-matrix_query3.json \
  --run --pretty
```
...
```bash
opscli skills install ops-refund-priority-matrix            # 安装
opscli skills install ops-refund-priority-matrix --force     # 强制重装
opscli skills status --pretty                                # 查看版本
opscli skills upgrade ops-refund-priority-matrix             # 升级
```
