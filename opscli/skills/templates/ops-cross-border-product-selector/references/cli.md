...
description: 使用 CLI 模式查询本地数据集索引并执行 跨境选品决策助手 数据查询
version: v0.1.0
---

# ops-cross-border-product-selector (CLI 模式)
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/cross-border-product-selector_query1.json \
  --run --pretty
```
...
opscli query build \
  ...
  --dimension asin \
  --output /tmp/cross-border-product-selector_query2.json \
  --run --pretty
```
...
opscli query build \
  --dataset 库存周转数据 \
  --dimension asin \
  --output /tmp/cross-border-product-selector_query3.json \
  --run --pretty
```
...
```bash
opscli skills install ops-cross-border-product-selector            # 安装
opscli skills install ops-cross-border-product-selector --force     # 强制重装
opscli skills status --pretty                                # 查看版本
opscli skills upgrade ops-cross-border-product-selector             # 升级
```
