#!/usr/bin/env python3
"""
Script Name: product_selector_mcp.py
Description: 跨境选品决策系统（MCP 模式 - 无 opscli 依赖）
Author: opscli Team
Date: 2026-04-29

Usage:
    echo '{"category":"Kitchen Gadgets",...}' | python product_selector_mcp.py
    python product_selector_mcp.py --input /tmp/selector.json --pretty
"""
...
from core import run_selection


def main() -> None:
    parser = argparse.ArgumentParser(description="跨境选品决策系统（MCP 无状态模式）")
    ...
    try:
        ...
        if not raw.strip():
            result = {"success": False, "error": "输入为空，请通过 stdin 传入 JSON 选品参数"}
