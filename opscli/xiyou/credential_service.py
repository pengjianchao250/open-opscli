...
class XiyouCredentialServiceClient:
    ...
    def get_latest(self) -> XiyouCredential:
        ...
        if not self.settings.credential_latest_url:
            raise XiyouConfigError("缂哄皯 OPSCLI_XIYOU_CREDENTIAL_LATEST_URL")
        ...
        if status_code >= 400:
            raise XiyouConfigError(f"鑾峰彇瑗挎煔鏈€鏂板嚟鎹け璐ワ細status={status_code} response={text[:500]}")
        try:
            payload = response.json()
        except Exception as exc:
            raise XiyouConfigError("鑾峰彇瑗挎煔鏈€鏂板嚟鎹け璐ワ細鍝嶅簲涓嶆槸 JSON") from exc
...
def parse_latest_credential_response(payload: Any) -> XiyouCredential:
    ...
    if not isinstance(payload, dict):
        raise XiyouConfigError("瑗挎煔鏈€鏂板嚟鎹搷搴斿繀椤绘槸 JSON 瀵硅薄")

    if payload.get("success") is False:
        error = payload.get("error")
        message = error.get("message") if isinstance(error, dict) else None
        raise XiyouConfigError(f"鑾峰彇瑗挎煔鏈€鏂板嚟鎹け璐ワ細{message or 'success=false'}")

    code = payload.get("code")
    if code not in (None, 0, 200, "0", "200"):
        msg = _optional_str(payload.get("msg"))
        raise XiyouConfigError(f"鑾峰彇瑗挎煔鏈€鏂板嚟鎹け璐ワ細{msg or f'code={code}'}")
...
def _parse_cookie_content(raw: str, *, fallback: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise XiyouConfigError("瑗挎煔鏈€鏂板嚟鎹噷鐨?cookie_content 涓嶆槸鏈夋晥 JSON") from exc
    if not isinstance(content, dict):
        raise XiyouConfigError("瑗挎煔鏈€鏂板嚟鎹噷鐨?cookie_content 缁撴瀯閿欒")
