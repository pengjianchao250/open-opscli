...
def _with_default_dataset_recommendation(result: dict, candidate: dict | None) -> dict:
    ...
    if (
        result.get("planner_status") == "candidate_ready"
        and existing
...
def _score_profile(
    ...
    return {
        ...
        "reasons": reasons,
        "grain_coverage": _extra_slot_terms(profile, slots, rules),
        "_semantic_rank": _semantic_rank(profile, domains, slots, rules, query),
    }
...
def load_authorized_cards(adapter: MetadataAdapter) -> list[dict]:
    """从元数据适配器构建并校验选表决策卡片。

    卡片在进程内即时构建（不落盘），构建后再过一遍卡片结构校验，
    保证进入规划器的数据满足选表所需的完整约束。
    """
    return _validate_cards(scoped_metadata_index.build_cards(adapter))
