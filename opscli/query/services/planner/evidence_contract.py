...
原样迁入（去 CLI 壳，只保留纯函数）：所有常量、辅助函数与 `build_evidence_contract`
的实现逻辑与源实现逐行一致，未做任何算法改动（行为等价铁律，见本次内核化收官计划
`global-constraints.md`）。CLI 入口（`argparse`/`main`/stdin 读取）不迁入——kernel
版直接以 Python 函数调用形式被 `entry.py:run_flow` 接入，不再需要 CLI 壳。
