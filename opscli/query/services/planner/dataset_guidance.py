...
输入：元数据适配器（后端 query-metadata 同形行）+ 数据集别名 + 用户查询（可选点名字段）。
输出：dataset_guidance_v1 规划器，包含字段选择（含公式/快照聚合口径）、
权限筛选范围（查询组件可用性）与下一步动作。
本模块只读当前授权元数据，由 query_plan 规划器调用。
"""
...
def _load_target_metadata(adapter: MetadataAdapter, lookup: dict) -> tuple:
    """从元数据适配器加载目标数据集的全部相关元数据，保证口径一致。

    返回：(dataset, datasets, fields, select_columns, all_fields, all_select_columns, fingerprint)
    其中 fields/select_columns 已按目标 dataset_alias 过滤；all_fields/all_select_columns
    为全量（供 _default_filters 回查组件字段、以及空数据集派生组件维度）。
    数据来源为后端 query-metadata 同形行，一次性取全量后在内存过滤，保证快照一致。
    """
    datasets = adapter.datasets_rows()
    dataset = _find_dataset(datasets, lookup)
    dataset_alias = dataset["dataset_alias"]
    all_fields = adapter.fields_rows()
    all_select_columns = adapter.select_columns_rows()
    fields = [row for row in all_fields if row["dataset_alias"] == dataset_alias]
    select_columns = [
        row for row in all_select_columns if row["current_dataset_alias"] == dataset_alias
    ]
...
def build_guidance(
    ...
    *,
    ...
    # 数据集定位统一走 dataset_alias（内部构造 lookup 复用 _find_dataset 校验语义）
    (
        dataset,
        datasets,
        fields,
        ...
        all_select_columns,
        fingerprint,
    ) = _load_target_metadata(adapter, {"dataset_alias": dataset_alias})
    dataset_alias = dataset["dataset_alias"]
    # _load_target_metadata 已返回全量字段（all_fields）与全量组件关系（all_select_columns），
    # 无需二次读取；空数据集（如权限枚举组件）从组件关系派生可枚举维度。
    if not fields:
