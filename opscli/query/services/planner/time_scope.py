...
# 该词表同时被 query_plan 的字段标签匹配复用（见 mask_negated_spans），
# 统一维护一份，避免时间口径与字段标签两处否定判定漂移。
_NEGATED_SPAN_RE = re.compile(
    r"(?:拒绝|排除|不要|不用|不加|不添加|不需要|无需|禁止|别用|别加|忽略|去掉|去除)"
    r"[^，。；！？,;!?]{0,12}"
)


def mask_negated_spans(query: str) -> str:
    """把否定语境整段替换为空格，供字段标签匹配等场景复用。

    为什么要暴露：字段标签匹配原先直接对原文做子串包含，
    「不加日期筛选」里的「日期」被当成用户点名的分组维度，
    结果是用户越明确要求不要日期，规划器越确定按日期分组（行数爆炸）。
    """
