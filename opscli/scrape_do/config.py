...
ENV_TOKEN_FILE = "OPSCLI_SCRAPEDO_TOKEN_FILE"
...
DEFAULT_TOKEN_FILE = CONFIG_DIR / "scrape_do" / "token.txt"
...
def load_settings() -> ScrapeDoSettings:
    ...
    return ScrapeDoSettings(
        ...
        token=values.get(ENV_TOKEN) or _read_token_file(values.get(ENV_TOKEN_FILE)) or None,
...
def _load_env_values() -> dict[str, str]:
    values = _read_dotenv()
    for key in [ENV_TOKEN, ENV_TOKEN_FILE, ENV_ACCOUNT_NAME, ENV_OUTPUT_DIR, ENV_TIMEOUT_SECONDS, ENV_ACCOUNT_CACHE_TTL_SECONDS]:
...
def _read_token_file(path_value: str | None = None) -> str | None:
    path = Path(path_value).expanduser() if path_value else DEFAULT_TOKEN_FILE
