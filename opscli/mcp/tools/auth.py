...
async def auth_login_poll(device_code: str, timeout: int = 10) -> dict:
    ...
    授权成功后自动将 session_id 保存到本地文件（~/.config/opscli/mcp_sessions.json），
    避免每次重启服务或新对话时重复授权。
...
async def auth_get_token(system: str = "ops", session_id: str | None = None) -> dict:
    ...
    try:
        ...
        # 自动保存 JWT 到本地缓存
        from opscli.mcp.session_store import save_jwt
...
async def auth_check_token(jwt: str | None = None, system: str = "ops") -> dict:
    ...
    target_jwt = jwt or get_jwt(system)
    if not target_jwt:
...
async def auth_logout(system: str = "__all__") -> dict:
    ...
    仅删除本地文件中的凭证，不影响后端 session 状态。

    Args:
        system: 系统别名，"__all__" 表示清除所有系统
    """
    try:
        from opscli.mcp.session_store import clear_all_sessions, remove_session

        if system == "__all__":
            clear_all_sessions()
            return _ok({"cleared": "all"})
        removed = remove_session(system)
        return _ok({"cleared": system, "existed": removed})
    except Exception as exc:
        return _err(exc)
...
async def auth_doctor(session_id: str | None = None) -> dict:
    ...
    try:
        client = _auth_client()
        from opscli.mcp.tools.helpers import _get_session_id
        from opscli.mcp.session_store import get_jwt, list_sessions
