...
<main>
  ...
  <section class="tab-panel" id="panel-accounts" role="tabpanel" aria-labelledby="tab-accounts" hidden>
    <div class="account-stack">
      ...
      <section class="panel"><h2>今日 MCP 功能调用</h2><p class="scope-note">读取统一遥测调用事实；执行与代理转发分行展示，避免重复合计。该表不判断业务成功或失败。</p><div class="data-table-wrap"><table><thead><tr><th>用户</th><th>服务</th><th>功能</th><th>角色</th><th>调用次数</th><th>平均 / 最大耗时</th><th>最近调用</th></tr></thead><tbody id="feature-usage-today"></tbody></table></div></section>
    </div>
  </section>
...
function renderAccounts(accountData,usageData,featureUsageData){
  const accountRows=accountData.accounts||[], usageRows=usageData.usage||[], featureRows=featureUsageData.usage||[];
  const accountError=accountData.source?.error?.message;
  const usageError=usageData.source?.error?.message;
  const featureError=featureUsageData.source?.error?.message;
  ...
  document.querySelector("#feature-usage-today").innerHTML=featureError?`<tr><td colspan="7" class="empty">${esc(featureError)}</td></tr>`:featureRows.map(u=>`<tr><td><strong>${esc(u.identity)}</strong></td><td><strong>${esc(u.service_label)}</strong><br><span class="muted">${esc(u.service)}</span></td><td>${esc(u.operation)}</td><td>${esc(u.runtime_role==="executor"?"执行":"代理转发")}</td><td>${esc(u.calls)}</td><td>${u.avg_duration_ms??"—"} / ${u.max_duration_ms??"—"} ms</td><td>${age(u.last_called_at)}</td></tr>`).join("")||'<tr><td colspan="7" class="empty">今日暂无 MCP 功能调用记录。</td></tr>';
}
async function refreshAccounts(){try{const [accounts,usage,featureUsage]=await Promise.all([json("/api/v1/accounts"),json("/api/v1/usage/today"),json("/api/v1/usage/tools/today")]);renderAccounts(accounts,usage,featureUsage);}catch(e){document.querySelector("#accounts").innerHTML='<tr><td colspan="6" class="empty">账号数据暂不可用。</td></tr>';document.querySelector("#usage-today").innerHTML='<tr><td colspan="7" class="empty">额度数据暂不可用。</td></tr>';document.querySelector("#feature-usage-today").innerHTML='<tr><td colspan="7" class="empty">功能调用数据暂不可用。</td></tr>';}}
